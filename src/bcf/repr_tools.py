...
from string templates. Used in computing the left and right vectors for ReFACT.
"""
...
def get_reprs_at_word_tokens(
    ...
    # for c, w, i in zip(context_templates, words, idxs):
    #     print(c.format(w), " , token to perform editing on:", tok.decode(tok(c.format(w))["input_ids"][i[0]]), i)
...
def get_words_idxs_in_templates(
    tok: CLIPTokenizer, context_templates: List[str], words: List[str], subtoken: str
) -> int:
    """
...
def get_reprs_at_idxs(
    ...
    def _process(cur_repr, batch_idxs, key):
        ...
        for i, idx_list in enumerate(batch_idxs):
            mean = cur_repr[i][idx_list].squeeze()
            to_return[key].append(mean)
    ...
    for batch_contexts, batch_idxs in _batch(n=128):
        ...
        with torch.no_grad():
            with nethook.Trace(
                ...
                model.get_text_features(**contexts_tok)
