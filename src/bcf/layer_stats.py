...
from src.util.nethook import Trace, set_requires_grad
from src.util.runningstats import CombinedStat, Mean, NormMean, SecondMoment, tally
...
def layer_stats(
    ...
    def tokenization(example):
        ...
        if batch_tokens is not None and batch_tokens < maxlen:
            maxlen = batch_tokens
        return tokenizer(example["text"], max_length=maxlen, truncation=True)
    ...
    def get_ds():
        ...
        # raw_ds = load_dataset("conceptual_captions", split="train")
        maxlen = model.config.max_position_embeddings
        # raw_ds = [x for x in raw_ds if x["caption"]]
        # for x in raw_ds:
            # x["text"] = x["caption"]

        raw_ds = [x for x in raw_ds if x["text"]]
        if batch_tokens is not None and batch_tokens < maxlen:
            maxlen = batch_tokens
    ...
    filename = f"{stats_dir}/{file_extension}"
